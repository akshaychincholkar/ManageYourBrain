  ...
  return (
    <div>
      <h2 id="tag-heading" data-cy="TagHeading">
        <Translate contentKey="manageYourBrainApp.tag.home.title">Tags</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="manageYourBrainApp.tag.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/tag/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="manageYourBrainApp.tag.home.createLabel">Create new Tag</Translate>
          </Link>
        </div>
      </h2>
        ...
        {tagList && tagList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="manageYourBrainApp.tag.id">Id</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="manageYourBrainApp.tag.name">Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                <th className="hand" onClick={sort('creationDate')}>
                  <Translate contentKey="manageYourBrainApp.tag.creationDate">Creation Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('creationDate')} />
                </th>
                <th className="hand" onClick={sort('modifiedDate')}>
                  <Translate contentKey="manageYourBrainApp.tag.modifiedDate">Modified Date</Translate>{' '}
            ...
            <tbody>
              {tagList.map((tag, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{tag.name}</td>
                  <td>{tag.creationDate ? <TextFormat type="date" value={tag.creationDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{tag.modifiedDate ? <TextFormat type="date" value={tag.modifiedDate} format={APP_DATE_FORMAT} /> : null}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="manageYourBrainApp.tag.home.notFound">No Tags found</Translate>
            </div>
          )
        )}
      </div>
